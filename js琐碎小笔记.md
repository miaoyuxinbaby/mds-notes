...
按照 JSON 的规范，使用 JSON.stringify() 做对象序列化时，如果一个属性为函数或undefined，那这个属性就会被忽略。如果属性为 null 则可以正常序列化这个属性，遇到循环引用就GG

ES6模块的设计思想，是尽量地静态化，使得编译时就能确定模块的依赖关系(“静态优化”)，以及输入和输出的变量。CommonJS和AMD模块，都只能在运行时确定这些东西(“运行时加载”)。
